...
The ToDo page is a checklist of leaf tasks whose planned date range includes
today.
...
- Show the complete parent path above each task when **Show full path** is
  enabled.
- Tasks are ordered by start time and then by name; tasks without a start time
  appear after timed tasks.
