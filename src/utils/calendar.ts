...
import { minutesBetweenDateTimes, timeToMinutes } from "./time.ts";
import type { CalendarItem } from "@/types/calendar";
import type { TaskNode } from "@/types/graph";

export const CALENDAR_START = 8 * 60;
export const CALENDAR_END = 20 * 60;
...
export function calendarItem(
  ...
  const duration =
    startDate === endDate
      ? Math.max(30, minutesBetweenDateTimes(startDate, startTime, endDate, endTime))
      : 60;
  return {
    task,
    startDate,
